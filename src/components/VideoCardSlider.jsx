  ...
  return (
    <section className="relative mx-auto w-full pb-[140px] md:pb-[130px]">
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-16 items-center pb-[15px] md:mt-[160px]">
        {/* LEFT TEXT */}
        <div className="px-5">
          <h2 className="text-[28px] font-medium leading-[118%] lg:text-[40px] mb-5">
            Remix everything
          </h2>
          <p className="text-[16px] font-medium leading-[140%] lg:text-[20px] lg:leading-[130%]">
            Take someone else’s creation and put your <br />
            spin on it. Swap characters, change the <br />
            vibe, add new scenes, or extend the story.
          </p>
        </div>

        {/* RIGHT VIDEO */}
        <div className="relative flex justify-center flex-col items-center gap-5 px-4">
          {/* HARD TEXT OVERLAY WITH FLOAT ANIMATION */}
          <motion.div
            key={index}
            animate={{ y: [-5, 5, -5] }}
            transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
            className="absolute left-10 md:-left-35 bottom-10 md:bottom-20 z-20 bg-gray-800/70  rounded-4xl p-2 sm:p-3 px-4 sm:px-6 text-white font-medium max-w-[400px] text-base  flex items-center space-x-2 sm:space-x-3 gap-4"
          >
            {videos[index].title}
            <motion.div
          ...
          {/* VIDEO */}
          <div
            className="relative rounded-3xl overflow-hidden border border-gray-700"
